...
public class ProxyListActivity extends BaseFragment implements NotificationCenter.NotificationCenterDelegate {
    ...
    public class TextDetailProxyCell extends FrameLayout {
        ...
        public void setProxy(SharedConfig.ProxyInfo proxyInfo) {
            String typeLabel;
            if (proxyInfo.proxyType == SharedConfig.PROXY_TYPE_VLESS) {
                typeLabel = "VLESS | ";
            } else if (proxyInfo.proxyType == SharedConfig.PROXY_TYPE_MTPROTO) {
                typeLabel = "MTProto | ";
            } else {
                typeLabel = "SOCKS5 | ";
            }
            textView.setText(typeLabel + proxyInfo.address + ":" + proxyInfo.port);
    ...
    public View createView(Context context) {
        ...
        listView.setOnItemClickListener((view, position) -> {
            if (position == useProxyRow) {
                ...
                ConnectionsManager.applyCurrentProxy(useProxySettings);
                ...
                editor.putBoolean("proxy_enabled", useProxySettings);
                if (!info.secret.isEmpty() || info.proxyType == SharedConfig.PROXY_TYPE_VLESS) {
                    useProxyForCalls = false;
    ...
    private void updateRows(boolean notify) {
        ...
        if (SharedConfig.currentProxy == null || (SharedConfig.currentProxy.secret.isEmpty() && SharedConfig.currentProxy.proxyType != SharedConfig.PROXY_TYPE_VLESS)) {
    ...
    private void checkProxyList() {
        for (int a = 0, count = proxyList.size(); a < count; a++) {
            ...
            if (proxyInfo.proxyType == SharedConfig.PROXY_TYPE_VLESS) {
                // For VLESS: start sing-box locally, then check the local SOCKS5 proxy
                new Thread(() -> {
                    VlessProxyService tempService = VlessProxyService.getInstance();
                    boolean wasRunning = tempService.isRunning();
                    if (!wasRunning) {
                        tempService.start(proxyInfo);
                    }
                    if (tempService.isRunning()) {
                        ConnectionsManager.getInstance(currentAccount).checkProxy(
                                "127.0.0.1", tempService.getLocalPort(),
                                tempService.getLocalUsername(), tempService.getLocalPassword(), "",
                                time -> AndroidUtilities.runOnUIThread(() -> {
                                    ...
                                    if (!wasRunning && SharedConfig.currentProxy != proxyInfo) {
                                        tempService.stop();
                                    }
